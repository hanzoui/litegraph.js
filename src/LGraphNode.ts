...
export class LGraphNode implements NodeLike, Positionable, IPinnable, IColorable {
  ...
  // TODO: Remove this temporary override when feature flag integration is complete
  static readonly FORCE_VUE_POSITIONING = true
   ...
   */
  #calculateVueSlotPosition(isInput: boolean, slot: INodeSlot, slotIndex: number): Point {
    const { pos: [nodeX, nodeY], size: [width] } = this
    ...
    const headerDifference = dimensions.HEADER_HEIGHT - LiteGraph.NODE_TITLE_HEIGHT
    ...
    if (isInput && isWidgetInputSlot(slot as INodeInputSlot)) {
      ...
      // Find widget index
      const widgetIndex = this.widgets?.findIndex(w => w.name === (slot as INodeInputSlot).widget?.name) ?? 0

      // Y position relative to the node body top (not the header)
      slotCenterY = headerDifference +
        (regularInputCount * dimensions.SLOT_HEIGHT) +
        (regularInputCount > 0 ? spacing.BETWEEN_SLOTS_AND_BODY : 0) +
        (widgetIndex * (dimensions.STANDARD_WIDGET_HEIGHT + spacing.BETWEEN_WIDGETS)) +
